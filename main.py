...
# Используйте переменные окружения или задайте значения напрямую (но для безопасности лучше через переменные)
DB_HOST = os.environ.get('DB_HOST', '45.153.71.178')   # публичный IP вашей БД (или приватный)
...
def get_db_connection():
    """Создаёт подключение к PostgreSQL с таймаутом 5 секунд."""
...
def init_tables():
    ...
    try:
        ...
        cur.close()
        conn.close()
        print("✅ Таблицы созданы/проверены", flush=True)
    except Exception as e:
        print(f"⚠️ Ошибка при создании таблиц: {e}", flush=True)

# Вызываем инициализацию таблиц (неблокирующая, ошибки ловятся)
init_tables()

# ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ====================
def check_auth():
    """Проверяет токен авторизации из заголовка Authorization."""
...
# ----- Вебхук от Zabbix (сохраняет сообщение) -----
...
# ----- Получение сообщений (с учётом прочитанных для пользователя) -----
